...
const { scanFileTs, scanFileVue, getJsonContent, getFileType } = require(path.join(__dirname, './file'));     // 读写模块
...
class CodeAnalysis {
  constructor(options) {
    ...
    this._tagList = options.tagList || [];                                             // 需要标记的API配置        
    ...
    this.httpRepoMap = {};                                                             // httpRepo映射Map     
    ...
    if(queue.length>0){
      queue.forEach((item)=>{
        Object.keys(this[item.mapName]).forEach((apiName)=>{
          if(this._tagList.length>0 && this._tagList.includes(apiName)){          // 标记API
            this[item.mapName][apiName].isTag = true;
          }
        })
      })
    }
  ...
  _runBrowserPlugins(tsCompiler, baseNode, depth, apiName, filePath, projectName, line) {
    if(this.browserQueue.length>0){
      for(let i=0; i<this.browserQueue.length; i++){
        const checkFun = this.browserQueue[i].checkFun;
        if(checkFun(this, tsCompiler, baseNode, depth, apiName, filePath, projectName, line)){
          break;
        }
      }
    }
  ...
  _findImportItems(ast, filePath, baseLine = 0) {
    ...
    function dealImports(temp){
      ...
      if (!that.importItemMap[temp.name]) {
        ...
        that.importItemMap[temp.name].as = temp.origin;
        that.importItemMap[temp.name].cF = [];
        that.importItemMap[temp.name].cF.push(filePath);
      } else {
        that.importItemMap[temp.name].cF.push(filePath);
      }
    }
        ...
        if(symbol && symbol.declarations){
          if(symbol.declarations.length>1 || ( symbol.declarations.length==1 && symbol.declarations[0].pos >ast.end)){                            // 在AST中找不到上下文声明，证明是Bom,Dom对象
            ...
            if(!(depth>0 && node.parent.name && node.parent.name.pos ==node.pos && node.parent.name.end ==node.end)){                             // 排除作为属性的场景
              that._runBrowserPlugins(tsCompiler, baseNode, depth, apiName, filePath, projectName, line);
            }
          }
        }
      }
  ...
  // 扫描文件
  _scanFiles(scanSource, _isScanVue) {
    let entrys = [];
    scanSource.forEach((item)=>{
      ...
      scanPath.forEach((sitem)=>{
        ...
        if(_isScanVue){
          const vueEntry = scanFileVue(sitem);
          tempEntry = tempEntry.concat(vueEntry);
        }
        let tempPath = tempEntry.map((titem)=>{
          if(item.format){
            return titem.substring(titem.indexOf(sitem)).replace(item.format, '');
          }else{
  ...
  // 扫描文件，分析代码
  _scanCode(scanSource, _isScanVue) {
    let entrys = this._scanFiles(scanSource, _isScanVue);
    // console.log(entrys);
    entrys.forEach((item, findex)=>{
      if(!this.httpRepoMap[item.name]){
        this.httpRepoMap[item.name] = item.httpRepo
      }
      ...
      if(parseFiles.length>0){
        parseFiles.forEach((element, eIndex) => {
          ...
          try {
            const type = getFileType(element);
            if(type === CODEFILETYPE.VUE){
              ...
              if(Object.keys(importItems).length>0 || this._browserApis.length>0){
                this._dealAST(importItems, ast, checker, showPath, item.name, baseLine);           // 递归分析AST，统计相关信息
              }
            }else if(type === CODEFILETYPE.TS || type === CODEFILETYPE.TSX){
            ...
            temp.forEach(element => {
              if (element == analysisTarget) {
                ...
                if (!this.versionMap[version]) {
                  ...
                  this.versionMap[version].cN = 1;
                  this.versionMap[version].cS = [];
                  this.versionMap[version].cS.push(item.name);
                } else {
                  this.versionMap[version].cN++;
                  this.versionMap[version].cS.push(item.name);
                }    
              }
            });
  ...
  analysis() {
    ...
    // 扫描分析代码文件
    this._scanCode(this._scanSource, this._isScanVue);
    // API标记
    this._apiTag(this.pluginsQueue);
    this._apiTag(this.browserQueue);
    // 目标依赖安装版本收集
